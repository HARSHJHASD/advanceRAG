...
// This application always supplies Gemini embeddings to Chroma explicitly.
// Providing this adapter prevents the client from attempting to load Chroma's
// optional DefaultEmbeddingFunction whenever it opens the collection.
const externalEmbeddingFunction = {
  name: "external-precomputed-embeddings",
  async generate() {
    throw new Error(
      "Embeddings must be generated by embeddingService before using Chroma."
    );
  },
};
...
export async function getDocumentIdsBySourceId(sourceId) {
  const collection = await getCollection();
...
export async function deleteDocumentsById(ids) {
  if (!ids.length) {
    return;
  }
